...
\begin{description}
	...
	\item [WHERE:] Both patterns $<$date\_inf$>$ and $<$date\_sup$>$ allow filtering the data based on the date range defined by the user at run time. The $<$custom\_filter$>$ allows to combine a predefined pattern, such as data status, or a user-defined constraint by value query\remPierre{``value query'' n'est pas clair pour moi}\ when enabled by filling the ``c\_metadataxml'' column. The $<$codes$>$ pattern can optionally specify a list of codes\remPierre{``codes'' plutôt que ``coding''? Ou sinon ``coding systems''?} (e.g: SNOMED, LOINC\ldots) by populating the i2b2 ontology ``c\_basecode'' column.
\end{description}
...
The challenges of \textit{data federation}, \textit{data interoperability}, \textit{data freshness} and \textit{data security} can benefit from both i2b2 experience and 
FHIR simplification.
...
In the end, while a tool that is able to bridge international institutions together is likely to emerge, concept mapping between many institutions remains to be done. Since \remPierre{all = these institutions?}these institutions are likely to use different languages, different granularities and different concepts and practices, this remains a challenge to be addressed. While ontology matching is an old research area, it is an area that still presents significant challenges to overcome.
